...
dependency was removed. What it measured is preserved in ``docs/excel-round-trip-findings.md``;
...
DATA_DIR: Path = Path(os.environ.get("PQX_FIXTURE_DIR") or Path(__file__).resolve().parents[2] / "data")
"""Where the fixtures land. Gitignored; rebuilt on demand rather than tracked.

Resolved out of the source tree deliberately: ``parents[2]`` is the package root, so the
files land in ``packages/pqx-testing/data/`` rather than beside this module inside
``src/``, where a generated 1000-row workbook would sit in the importable package and be
swept up by any tool that walks it.

``PQX_FIXTURE_DIR`` overrides it. That is not decoration: ``parents[2]`` is only correct
for an editable install from the workspace, which is how this dev-only package is always
installed, and the variable is the escape hatch if that ever stops being true.
"""
